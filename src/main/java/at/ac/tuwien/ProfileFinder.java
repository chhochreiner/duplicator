	...
	public ProfileFinder() {
		...
		final Button submit = new AjaxButton("submit", queryForm) {
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> queryForm) {
				...
				if (apiService.checkNetworkId("googleplus_id", uuid)) {
					result.addOrReplace(new Label("googleplus_image", "<img src=\"/ok.png\"/>").setEscapeModelStrings(false));
				} else {
					result.addOrReplace(new Label("googleplus_image", "<img src=\"/fail.png\"/>")
					.setEscapeModelStrings(false));
				}
				...
				GooglePlusUser = new WebMarkupContainer("GooglePlusUser").add(new SimpleAttributeModifier("src",
						apiService.excecuteGooglePlusQuery(uuid)));
		...
		XingUser.setOutputMarkupId(true);
		GooglePlusUser.setOutputMarkupId(true);
		result.add(linkedInUsers, twitterUsers, facebookUsers, XingUser, GooglePlusUser);
